...
function groupQuantityByDate(transactions: Transaction[]): QuantityByDate[] {
  const result: Record<
    string,
    { productQuantity: number; serviceQuantity: number }
  > = {};

  transactions.forEach((tx) => {
    const dateKey = tx.date.toISOString().split("T")[0] ?? "";

    result[dateKey] ??= { productQuantity: 0, serviceQuantity: 0 };

    result[dateKey].productQuantity += tx.productQuantity;
    result[dateKey].serviceQuantity += tx.serviceQuantity;
  });

  return Object.entries(result).map(
    ([date, { productQuantity, serviceQuantity }]) => ({
      date,
      productQuantity,
      serviceQuantity,
    }),
